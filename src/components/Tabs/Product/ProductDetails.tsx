...
function ProductDetails() {
  ...
  const onOptionChange = (event: any) => {
    const childNodes = event.target && event.target.parentNode.childNodes;
    for (let i = 0; i < childNodes.length; i++) {
      if (childNodes[i].className === "product__option-description") {
        store.updateSelectedProductOption(childNodes[i].innerText);
        break;
      }
    }
  }
  ...
  return <div> {
    data && products.map((item: any) => item.productName).includes(data.productName) ?
        ...
        <div className='product__summary-options'>
          {
            data.option1 && data.option2 ?
              <div>
                <section className='product__option'>
                  <input type="radio" onChange={(e) => onOptionChange(e)} name="option" id="first" />
                  <label htmlFor="first">Option 1</label>
                  <p className='product__option-description'>{data.option1}</p>
                </section>
                <section className='product__option'>
                  <input type="radio" onChange={(e) => onOptionChange(e)} name="option" id="second" />
                  <label htmlFor="second">Option 2</label>
                  <p className='product__option-description'>{data.option2}</p>
                </section>
              </div>
              :
